...
We can also look at this from another perspective. The equivalency principle tells us that if you’re in a sealed box, it’s impossible to know if you’re falling to the ground or travelling towards Mars. Both _feel_ the same. We need an external point of reference to know if we’re going in the right direction, or plummeting to the ground.
...
That external point of reference is quite easy to understand: is the patch you (or one of the developers on your team) working on right now solving an actual problem? If not, then it’s going in the _wrong_ direction. Code that pushes a project in the wrong direction is called _technical debt_.

### Amdahl’s Law

Amdahl’s Law is how computer scientists calculate the maximum speedup provided by adding

If your process runs on four cores and 25% of the time is spent in some kind of mutex situation where the cores have to stop and wait for an operation to finish before work can continue, then adding more cores doesn’t give you any speedup.

We can translate this to team structures. The more a team needs to stop for consensus, the less work they get done.
...
Teams working at Amazon are generally limited to however many people can be fed with two pizzas — 6 to 12 people. This is more than most companies can handle on one team before efficiency starts dropping, and the reason why is Amazon’s management structure requires less upfront consensus than most.

Most people who’ve looked at this two-pizza rule, and most people who work at Amazon, seem to think this is because smaller teams are more efficient. This is not actually true.
...
To continue with our Amazon reference, small and independent teams at Amazon clearly define what service they are going to provide, and other teams can choose to consume these services (or not). The structure ends up being a loose coalition of small teams which both provide services to other teams and consume services provided by them.
...
So what comes first? The organisational structure, or the structure of the software?

Neither. The structure of the of the software _is_ the structure of the organisation. The structure of the organisation _is_ the structure of the software.

You can fight Conway’s Law by spending more money. But this is inefficient and companies that do this are generally killed by the market. <explain why>

If you want to create scalable and distributed software, you need a scalable and distributed development team.

### A Brave New World

**Cost gravity** tells us we are heading towards _extreme_ levels of scale and decentralisation. **Conway’s Law** says the most efficient way to build these systems is by having an organisational structure that is _extremely_ scalable and decentralised. In 10 years, our systems will be 32x larger and 32x more decentralised than today, and so will the dominant organisational structure. Whichever structure can operate at these levels of scale and decentralisation will be the most efficient structure, and therefore the most dominant.
...
The **Pareto distribution** and **equivalency principle** tell us that if you’re building software that you want other people to use, it has to solve a problem for them and it has to do it in the simplest and cheapest possible way. Code that doesn’t solve a problem is pushing your codebase _away_ from this goal, not towards it. Code that pushes you away from your goal is called technical debt.

So what about if we combine Amdah’ls Law with this need for all code to solve a problem. What happens if you make a rule that all pull requests to your codebase must solve a problem? Is it possible that a rule like this could also eliminate the need for upfront consensus?

If all pull requests solve some small problem, would they all come together to solve a much larger problem?
...
After implementing these simple rules, your software can grow through a process of evolution — a hill climbing algorithm. They allow developers to work with no upfront consensus of any kind and instead sets your project off on a _drunken stumble to greatness_. No meetings, no planning, no roadmaps, no managers, no code reviews, which means the organisation does not have any blocking processes or mutex situations preventing it from scaling.

Conversely, this protocol forces the project owner to relinquish control. Central control requires upfront consensus, which doesn’t scale unless you have an an effectively unlimited budget and are not working within the constraints of the free market.
...
You can start using this protocol in your own projects right now. The protocol can be found at [https://github.com/socialarchitecture/protocols/blob/master/SAIP1.MD](https://github.com/socialarchitecture/protocols/blob/master/SAIP1.MD "https://github.com/socialarchitecture/protocols/blob/master/SAIP1.MD") Instructions on how to use it can be found at [http://socialarchitecture.science](http://socialarchitecture.science) along with my recent research and experiments in collective code construction.

***

Problems are always solved more accurately and more efficiently as distance between the point of discovery and the design of the solution is reduced. The _norm_ will be for problems to be solved This gives every How does the company you currently work for compete when almost everyone is an entrepreneur; when anyone can contribute to the global economy by solving a _tiny_ problem that they are _passionate_ about, and this global army of problem solvers every imaginable problem in the most efficient possible way?
...
In the current paradigm, we’re told that central planning and intelligent design is the only way we can reliably create good software. This is provably false.

Not only is it possible to build software through a process of evolution by natural selection, but building software any other way will soon be unprofitable.

I use software as a case study, but don’t worry if you’re not that way inclined, the trends affect everything from sending rockets to Mars, to broadcast media and the English Premier League.